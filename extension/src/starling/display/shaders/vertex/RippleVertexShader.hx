...
class RippleVertexShader extends AbstractShader
{
    public function new()
    {
        super();
    ...
    {
        var phase : Float = Math.round(haxe.Timer.stamp() * 1000) / 200;
        var frequency : Float = 0.02;
        var amplitude : Float = 5;
        context.setProgramConstantsFromVector(Context3DProgramType.VERTEX, firstRegister, [phase, frequency, amplitude, 1]);
    }
}
